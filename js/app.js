...
const selectorSpinner = document.querySelector(".spinner-usar");
...
document.addEventListener("DOMContentLoaded", async () => {
  mostrarSpinner();
  //Se ejecuta la peticion al servidor y se espera su respuesta
  await runFetch();

  //Se ejecuta luego de dos segundos para que el spinner dure más
  setTimeout(() => {
    ocultarSpinner();
    //Una vez lista se muestran los elementos en la tabla
...
function mostrarTabla(personas) {
  ...
  if (personas.length === 0) {
    //Solo se ejecuta cuando el valor exista ningun valor para evitar repeticiones  
    if (document.getElementsByClassName("error-contenido").length <= 0) {
      const mensajeError = document.createElement("p");
      mensajeError.classList.add("error-contenido");
...
//Se modifica el css para que se muestre o no
function mostrarSpinner() {
  selectorSpinner.style.display = "block";
}
